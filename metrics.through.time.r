...
trop.metrics = array(-999,c(length(trop.sims),100,27));
temp.metrics = array(-999,c(length(temp.sims),100,27));
...
metric.names = c('global.richness','r.env.rich', 'gamma.stat','r.MRD.rich')
for (j in 1:4) {
  curr.metric = metric.names[j]
  plot(trop.metrics[1, , 'time']/1000, metrics[1, , curr.metric], xlim = c(0, max(trop.metrics[,,'time']/1000)), 
