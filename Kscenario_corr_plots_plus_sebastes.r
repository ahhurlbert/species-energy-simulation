...
#Plot
pdf(paste(analysis_dir,'/summaryplots/latcorr_subclades_vs_cladeAgeRich_',Sys.Date(),'_log.pdf',sep=''), height=10, width = 14)
...
mtext("(c)", 2, at = 3.6, outer = F, cex = cexabc, las = 1, line = 5)
par(new=T)
ymax = 1.2
...
plot(log10(lat.corr.output$clade.richness), lat.corr.output$r.lat.rich, ylim = c(-1,1), las=1, cex.axis = cexaxis, cex.lab = cexlab,
     xlab = expression(paste(plain(log)[10]," Clade richness")), ylab = "", pch = 16, cex = cexpts.seb)
