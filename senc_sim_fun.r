...
senc_sim_fun = function(sim.matrix,sim) {
	...
	reg.of.origin = sim.matrix$reg.of.origin[sim.matrix$sim.id == sim]; 
	w = sim.matrix$w[sim.matrix$sim.id == sim]; 
	alpha = sim.matrix$alpha[sim.matrix$sim.id == sim]; 
	beta = sim.matrix$beta[sim.matrix$sim.id == sim]; 
	sigma_E = sim.matrix$sigma_E[sim.matrix$sim.id == sim]; 
	carry.cap = sim.matrix$carry.cap[sim.matrix$sim.id == sim]; 
	energy.gradient = sim.matrix$energy.gradient[sim.matrix$sim.id == sim];  
	max.K = sim.matrix$max.K[sim.matrix$sim.id == sim];
