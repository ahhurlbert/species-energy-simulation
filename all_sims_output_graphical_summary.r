...
boxplot(log10(output$extant.S) ~ output$carry.cap, ylab = "Log10 Number of extant species", boxwex = boxwidth,col = 'orange2')

boxplot(log10(output$extinct.S) ~ output$carry.cap,ylab = "Log10 Number of extinct species", boxwex = boxwidth, col = 'yellow4')

boxplot(output$skipped.clades ~ output$carry.cap, boxwex = boxwidth, ylab = "Number of skipped clades", col='skyblue')
...
for (i in 4:7) {
  for (y in ys) {
    plot(output[output$carry.cap=="off",y] ~ jitter(output[output$carry.cap=="off",i]), ylab = names(output)[y], 
         xlab="", col='darkblue',pch=2, cex=1.5, ylim = range(output[,y],na.rm=T))
    points(output[output$carry.cap=="on",y] ~ jitter(output[output$carry.cap=="on",i]), 
